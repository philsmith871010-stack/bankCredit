...
Extraction: KM1 tables from PDFs with Claude Sonnet 5 using a fixed JSON schema and the template's row codes, with rules-based validation (ratios within plausible ranges, capital amounts consistent with ratios and RWA, period continuity against the prior quarter) and a human review queue for anything failing validation. News labelling with Claude Haiku 4.5 using a fixed event taxonomy. Both calls logged with prompt version and model for reproducibility.
...
## 9. Front end and hosting

Recommendation, to be confirmed against PWLBtoday's current stack: a server-rendered React application (Next.js, TypeScript) inside the PWLBtoday shell, reading Postgres through a thin API layer, with incremental regeneration for the board and profiles so pages are fast and cacheable. Charts as inline SVG components shared with the PDF exporter (server-side rendering of the same components to print). If PWLBtoday is not on React, the same design and API layer work with its templating; the charts become a small standalone script.

Hosting: Vercel or Cloudflare Pages for the front end, Neon or Supabase for Postgres, Cloudflare R2 for documents, GitHub Actions for jobs. All within free tiers at this scale.

Accounts: reuse PWLBtoday accounts for watchlists, saved views, alerts and the export. Public pages remain public.
...
1. Confirm PWLBtoday's front-end stack and hosting so the shell integration is planned once.
2. Name: "Counterparty" inside PWLBtoday, and whether to reserve a standalone domain now.
3. Accounts: reuse PWLBtoday sign-in for watchlists and alerts.
4. Who reviews the daily brief and the extraction queue, and the sign-off rule for publishing a score change.
5. Whether the Mac browser job is acceptable for the seven bot-blocked sites, or a small VPS is preferred.
