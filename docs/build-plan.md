...
- **Jobs.** GitHub Actions runs the daily, weekly and quarterly schedules and commits results, then rebuilds and deploys Pages. The seven bot-blocked sites (Lloyds, QNB, ADCB, Revolut, Investec, Norinchukin, State Street) are fetched by a weekly launchd job on your Mac running headless Chromium, which pushes the PDFs to the repo; the site shows a "manual refresh" badge on those banks if the job has not run.
