...
Built and running: the Pillar 3 collector with per-firm locators for 70 firms (`bankcredit/adapters/pillar3_locators.py`), the FCA NSM poll matched by LEI, the KM1 extractor, the documents table, the review queue, the local review skill and Mac runner, the unverified badge and the documents card on the Status page. First run collected 150 documents across UK banks, building societies, Australia and Canada. Firms whose PDFs carry no extractable text (Coventry) or no KM1 table (TSB quarterlies, pre-2022 small-society documents) are queued for the local skill; Lloyds needs the browser.
